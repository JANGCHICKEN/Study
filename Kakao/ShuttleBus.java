...
public class ShuttleBus {
	...
	public static void main(String[] args) {
		...
		//n, t, m 배열
		int ntm[][] = { { 1, 1, 5 }, { 2, 10, 2 }, { 2, 1, 2 }, { 1, 1, 5 }, { 1, 1, 1 }, { 10, 60, 45 } };
		//크루가 대기열에 도착하는 시각
		String timetable[][] = { { "08:00", "08:01", "08:02", "08:03" }, { "09:10", "09:09", "08:00" }, { "09:00", "09:00", "09:00", "09:00" }, { "00:01", "00:01", "00:01", "00:01", "00:01" }, {
		...
		for (int i = 0; i < ntm.length; i++) {
			System.out.println("answer : " + Answer(ntm[i], timetable[i]));
		}
	}

	public static String Answer(int ntm[], String timetable[]) {
		...
		int index = 0; //timetable 인덱스
		int conTime = 0;

		int time[] = new int[timetable.length];

		for (int i = 0; i < time.length; i++) {
			time[i] = Integer.parseInt(timetable[i].substring(0, 2)) * 60 + Integer.parseInt(timetable[i].substring(3, 5));
		}

		Arrays.sort(time);

		for (int i = 0; i < n; i++) { //운행 회차만큼 반복
			seki = m; //한 회차에 탈 수 있는 좌석 수
		...
		String answer = StringUtils.leftPad(String.valueOf((conTime/60)), 2, "0") + ":" + StringUtils.leftPad(String.valueOf((conTime%60)), 2, "0");
