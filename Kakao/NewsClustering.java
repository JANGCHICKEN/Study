...
public class NewsClustering {
	...
	public static void main(String[] args) {
		...
		String A[] = {"FRANCE", "handshake", "aa1+aa2", "E=M*C^2"};
		String B[] = {"french", "shake hands", "AAAA12", "e=m*c^2"};

		for(int i=0; i<4; i++)
		System.out.println(JaccardSimilarity(A[i], B[i]));

	}

	public static int JaccardSimilarity(String A, String B) {
		ArrayList<String> MultisetA = new ArrayList<String>();
		ArrayList<String> MultisetB = new ArrayList<String>();

		Pattern pattern = Pattern.compile("^[a-zA-Z]{2}$");
		Matcher matcher;

		for (int i = 0; i < A.length() - 1; i++) {
			matcher = pattern.matcher(A.substring(i, i + 2));
			if (matcher.find())
				MultisetA.add(A.substring(i, i + 2));
		}
		for (int i = 0; i < B.length() - 1; i++) {
			matcher = pattern.matcher(B.substring(i, i + 2));
			if (matcher.find())
				MultisetB.add(B.substring(i, i + 2));
		}
		...
		if (MultisetA.size() == 0 && MultisetB.size() == 0) {
			return 65536;
		} else {
			for (int i = 0; i < MultisetA.size(); i++) {
				for (int j = 0; j < MultisetB.size(); j++) {
					if (MultisetA.get(i).equalsIgnoreCase(MultisetB.get(j))) {
						intersection++;
						MultisetB.remove(j);
						break;
					}
			...
			double result = intersection / (double)(MultisetA.size()+MultisetB.size());			
